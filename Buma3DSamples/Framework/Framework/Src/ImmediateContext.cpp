...
{
public:
    CommandExecutionScope(ImmediateContext* _ictx) : ctx{ _ictx->ctx } { ctx.Begin(); }
    ~CommandExecutionScope()
    {
        ctx.End(ctx.GetGpuWaitFence());
        //ctx.Wait(ctx.GetGpuWaitFence());
        ctx.WaitOnCpu();
        ctx.MakeVisible();
    }
private:
...
ImmediateContext::ImmediateContext(DeferredContext& _ctx)
    : ctx{ _ctx }
{
}

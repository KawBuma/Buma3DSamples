...
bool HelloTexture::CreateDescriptorSetLayout()
{
...
{
    util::DescriptorSetAllocateDesc allocate_desc(BACK_BUFFER_COUNT + BACK_BUFFER_COUNT - 1);

    // buffer_descriptor_sets,texture_descriptor_setsをBACK_BUFFER_COUNT数分割り当てます。
    buffer_descriptor_sets.resize(BACK_BUFFER_COUNT);
    texture_descriptor_sets.resize(BACK_BUFFER_COUNT);

    {
        allocate_desc.SetNumDescriptorSets(BACK_BUFFER_COUNT + BACK_BUFFER_COUNT - 1);
        ...
        allocate_desc.SetDescriptorSetLayout(BACK_BUFFER_COUNT + 1, texture_layout.Get());
        ...
        texture_descriptor_sets[1] = dst_sets[BACK_BUFFER_COUNT];
        texture_descriptor_sets[2] = dst_sets[BACK_BUFFER_COUNT + 1];
    }
    ...
    // コピー用プールから割り当て
    {
        allocate_desc.Reset()
            ...
            .SetDescriptorSetLayout(0, texture_layout.Get())
            .Finalize();
